...
Принцип единственной ответственности - Весь мой кот разделен на классы в которых действия и ответственость у каждого своя например SettingRandomGame и RandomGame ;
...
Принцип разделения интерфейса - Все интнрфейсы разделены на более мелкие модули, каждый по своему функционалу;
Принцип открытости/закрытости - У меня создан интерфейс который позволяет создавать разные реализации игры (объекты открыты для расширения, но закрыты для модификации»), сейчас консоль , можно в будующем создать для WPF ;
